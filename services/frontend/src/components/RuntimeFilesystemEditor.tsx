...
// Nothing on a running device's filesystem is structurally protected the way lab.conf is. Module-
...
export function RuntimeFilesystemEditor({ labName, detail, preferredMachine = null }: RuntimeFilesystemEditorProps) {
  ...
  const latestRef = useRef({ handleNewFile, handleDownload, handleDelete, ensureLoaded, selectDir, selectFile, handleRename, handleTreeMove });
  latestRef.current = { handleNewFile, handleDownload, handleDelete, ensureLoaded, selectDir, selectFile, handleRename, handleTreeMove };

  const rowActions = useMemo<RowActions>(
    () => ({
      onNewFile: (dir) => void latestRef.current.handleNewFile(dir),
      onDownload: (path) => void latestRef.current.handleDownload(path),
          ...
          <>
            <div className="d-flex gap-2 mb-2 flex-wrap">
              ...
              <span
                ...
                title="Drag files/folders onto a folder to move them. Double-click or F2 to rename."
                aria-label="Drag files/folders onto a folder to move them. Double-click or F2 to rename."
              >
                <Info size={16} />
              </span>
...
// Memoized so a render of the parent that doesn't touch RowActionsCtx/setContextMenu (both
// stabilized above) doesn't force every visible row to re-render.

...
function toFormState(machine: MachineDetail): FormState {
  return {
    image: machine.image ?? "",
    mem: machine.mem ?? "",
...
function toPayload(form: FormState): MachineUpdatePayload {
  return {
...
export function MachineOptionsEditor({ show, labName, machine, deployed, onClose, onSaved }: MachineOptionsEditorProps) {
  ...
  useEffect(() => {
    if (machine) {
      const seeded = toFormState(machine);
      setForm(seeded);
      setInitial(seeded);
    } else {
      setForm(null);
  ...
  useEffect(() => {
    if (!show || netSysctls.length) return;
  ...
  async function handleSave() {
    ...
    await runBusy(setBusy, "Save device options", async () => {
      await api.updateMachine(labName, machine.name, toPayload(form));

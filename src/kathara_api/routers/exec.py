...
@router.websocket("/exec/live/ws")
async def exec_live_ws(
    websocket: WebSocket,
    lab_name: str,
    machine_name: str,
    service: KatharaService = Depends(get_service),
):
    """WebSocket live terminal transport for command streaming.

    Client messages:
      - {"type": "run", "command": <str|list[str]>, "wait": <bool>}
      - {"type": "close"}

    Server messages:
      - {"event": "output", "stream": "stdout"|"stderr", "data": "<base64>"}
      - {"event": "exit", "exit_code": <int>}
      - {"event": "error", "detail": "..."}
      - {"event": "ready"}
    """
    ...
    try:
        while True:
            ...
            if not isinstance(command, (str, list)):
                await _ws_send_error(websocket, "`command` must be a string or list.")
                continue

            stream = None
            try:
                stream = await run_in_threadpool(
                    service.exec_stream, lab_name, machine_name, command, wait
                )

                async for chunk in iterate_in_threadpool(_iter_exec_stream(stream)):
                    for stream_name, data in _output_parts(chunk):
                        await websocket.send_text(
                            json.dumps({"event": "output", "stream": stream_name, "data": _b64(data)})
                        )

                await websocket.send_text(json.dumps({"event": "exit", "exit_code": stream.exit_code()}))

...
from tests.helpers import FakeFacadeBase, lab_id, make_lab, make_service, register_lab, zip_bytes
...
class _FakeFacade(FakeFacadeBase):
    ...
    def exec(self, machine_name, command, lab_hash=None, wait=False, stream=False):
        ...
        self.exec_labs.append(lab_hash)
        return (b"", b"", 0)
...
def test_fresh_deploy_materializes_pending_to_native_fs_not_exec(tmp_path):
    ...
    service.deploy_lab(lab_id(service, "lab1"))
...
def test_deploy_materializes_offline_edits_to_native_fs(tmp_path):
    ...
    service.fs_write_text_offline(lab_id(service, "lab1"), "/pc1/etc/motd", "hi\n")
    service.fs_mkdir_offline(lab_id(service, "lab1"), "/pc1/etc/empty")
...
def test_deploy_is_scoped_to_selected_machines(tmp_path):
    ...
    service.deploy_lab(lab_id(service, "lab1"), selected_machines={"pc1"})
...
def test_redeploy_of_already_running_machine_pushes_live_update_once(tmp_path):
    ...
    service.deploy_lab(lab_id(service, "lab1"))  # fresh: native materialization, no copy_files/exec
    service.deploy_lab(lab_id(service, "lab1"))  # both already running, nothing changed since: no push

    facade = service._instance
    assert facade.exec_calls == []
    assert facade.copied == []

    service.fs_write_text_offline(lab_id(service, "lab1"), "/r1.startup", "ip a\necho again\n")
    service.deploy_lab(lab_id(service, "lab1"))
...
def test_an_edit_made_before_the_first_deploy_is_not_pushed_again_later(tmp_path):
    ...
    service.fs_write_text_offline(lab_id(service, "lab1"), "/r1.startup", "echo before\n")
...
def test_live_push_runs_every_exec_against_its_own_lab(tmp_path):
    ...
    make_lab(service, "lab1", {"lab.conf": LAB_CONF}, [])
    service.deploy_lab(lab_id(service, "lab1"))
    service.fs_write_text_offline(lab_id(service, "lab1"), "/r1/etc/motd", "hi")
    service.fs_write_text_offline(lab_id(service, "lab1"), "/r1.startup", "echo again\n")

    service.deploy_lab(lab_id(service, "lab1"))

    facade = service._instance
    assert [cmd for _, cmd, _ in facade.exec_calls] == ["mkdir -p /etc", "sh /tmp/.kathara_boot.sh"]
    assert facade.exec_labs == [lab_id(service, "lab1")] * 2
...
def test_boot_script_treats_an_unreadable_startup_as_empty(tmp_path):
    ...
    lab = service.registry.get(lab_id(service, "lab1"))
...
def test_a_partial_redeploy_pushes_only_the_selected_running_device(tmp_path):
    ...
    make_lab(service, "lab1", {"lab.conf": LAB_CONF}, [])
    service.deploy_lab(lab_id(service, "lab1"))
    service.fs_write_text_offline(lab_id(service, "lab1"), "/r1.startup", "echo r1\n")
    service.fs_write_text_offline(lab_id(service, "lab1"), "/pc1.startup", "echo pc1\n")

    service.deploy_lab(lab_id(service, "lab1"), selected_machines={"r1"})
...
def test_live_push_recreates_device_dirs_and_pushes_text_files_only(tmp_path):
    ...
    service.fs_write_text_offline(lab_id(service, "lab1"), "/r1/etc/frr/frr.conf", "hostname r1\n")
    service.fs_upload_bytes_offline(lab_id(service, "lab1"), "/r1/opt/blob.bin", b"\xff\xfe\x00")
    service.deploy_lab(lab_id(service, "lab1"))
...
def test_offline_fs_writes_accumulate_without_clobbering(tmp_path):
    ...
    service.fs_write_text_offline(lab_id(service, "lab1"), "/pc1/a", "1")
    service.fs_write_text_offline(lab_id(service, "lab1"), "/pc1/b", "2")
...
def test_update_lab_conf_rebuilds_empty_lab(tmp_path):
    ...
    lab = service.update_lab_conf(lab_id(service, "lab1"), LAB_CONF)
    ...
    assert set(service.get_lab_or_reconstruct(lab_id(service, "lab1")).machines.keys()) == {"r1", "pc1"}
...
def test_update_lab_conf_preserves_existing_device_startup(tmp_path):
    ...
    service.update_lab_conf(lab_id(service, "lab1"), "r1[image]=kathara/base\nr1[0]=A\n")

    assert service.get_startup_scripts(lab_id(service, "lab1"))["r1"].strip() == "ip a"


def test_update_lab_conf_rejects_when_deployed(tmp_path):
    service = _service(tmp_path)
    service.create_lab(LabCreate(name="lab1"))
    service.update_lab_conf(lab_id(service, "lab1"), LAB_CONF)
    service.deploy_lab(lab_id(service, "lab1"))  # fake facade sets api_object on machines

    with pytest.raises(LabConfLockedError):
        service.update_lab_conf(lab_id(service, "lab1"), "pc1[image]=kathara/base\n")
...
def test_remove_machine_removes_from_model_and_lab_conf(tmp_path):
    ...
    service.remove_machine(lab_id(service, "lab1"), "r1")
...
def test_connect_disconnect_stopped_persists_lab_conf(tmp_path):
    ...
    service.connect_machine(lab_id(service, "lab1"), "pc1", "A", interface_number=0)
    conf = (service.store.lab_dir("lab1") / "lab.conf").read_text()
    assert conf == "pc1[0]=A\npc1[image]=kathara/base\n"
...
def test_add_machine_persists_to_lab_conf_and_defers_deploy_when_stopped(tmp_path):
    ...
    service._reload_lab_from_disk(service.store.lab_dir("lab1"))
...
def test_update_machine_persists_to_lab_conf_when_stopped(tmp_path):
    ...
    machine = service.update_machine(lab_id(service, "lab1"), "pc1", spec)
    ...
    # ...and it survives a reload from disk (truly persisted, not just live).
    service._reload_lab_from_disk(service.store.lab_dir("lab1"))
    reloaded = service.registry.get(lab_id(service, "lab1")).machines["pc1"]
...
def test_update_machine_unchanged_round_trip_keeps_an_imported_volume(tmp_path):
    ...
    detail = serializers.machine_to_detail(service.get_lab_or_reconstruct(lab_id(service, "lab1")).get_machine("pc1"))
    ...
    service.update_machine(lab_id(service, "lab1"), "pc1", unchanged)
...
def test_update_machine_rejects_when_deployed(tmp_path):
    ...
    with pytest.raises(LabConfLockedError):
        service.update_machine(lab_id(service, "lab1"), "pc1", MachineUpdate(mem="256m"))
...
def test_update_machine_clears_options_not_resubmitted(tmp_path):
    ...
    service.update_machine(lab_id(service, "lab1"), "pc1", MachineUpdate())
...
def test_runtime_interface_change_never_leaks_into_lab_conf(tmp_path):
    ...
    service.connect_machine(lab_id(service, "lab1"), "pc1", "B")
    ...
    service.connect_machine(lab_id(service, "lab1"), "pc2", "C", interface_number=1)
    conf = (service.store.lab_dir("lab1") / "lab.conf").read_text()
...
def test_disconnect_stopped_device_renumbers_and_lab_still_reloads(tmp_path):
    ...
    service.disconnect_machine(lab_id(service, "lab1"), "pc1", "B")  # remove the middle interface (eth1)
    ...
    pc1 = service.registry.get(lab_id(service, "lab1")).machines["pc1"]
    ...
    assert fresh.registry.get(lab_id(fresh, "lab1")) is not None
...
def test_edit_on_folder_based_lab_generates_lab_conf_first(tmp_path):
    ...
    spec = MachineCreate.model_validate({"name": "pc2", "image": "kathara/base"})
    service.add_machine(lab_id(service, "lab1"), spec)
...
def test_edit_on_lab_without_directory_is_skipped(tmp_path):
    ...
    register_lab(service, built)

    service.add_machine(lab_id(service, "live"), MachineCreate.model_validate({"name": "pc2", "image": "kathara/base"}))
...
def test_full_undeploy_restores_configuration_topology(tmp_path):
    ...
    service = _service(tmp_path)
    make_lab(service, "lab1", {"lab.conf": "pc1[image]=kathara/base\npc1[0]=A\n"}, [])
    service.deploy_lab(lab_id(service, "lab1"))  # fake facade sets api_object on pc1

    # Runtime add on the running pc1 → live model gains eth1→B (not persisted to lab.conf).
    service.connect_machine(lab_id(service, "lab1"), "pc1", "B")
    live = service.registry.get(lab_id(service, "lab1")).machines["pc1"]
    ...
    service.undeploy_lab(lab_id(service, "lab1"))
    pc1 = service.get_lab_or_reconstruct(lab_id(service, "lab1")).get_machine("pc1")

...
class _RuntimeFacade(FakeFacadeBase):
    ...
    def exec(self, machine_name, command, lab_hash=None, wait=False, stream=False):
        self.execs.append((machine_name, command, lab_hash, wait))
    ...
    def get_machine_api_object(self, machine_name, lab_hash=None):
        return ("api-object", machine_name, lab_hash)
...
@pytest.mark.parametrize(
    "call",
    [
        lambda s: s.fs_list_directory(lab_id(s, "l"), "pc2", "/"),
        lambda s: s.fs_read_bytes(lab_id(s, "l"), "pc2", "/etc/hosts"),
        lambda s: s.fs_write_text(lab_id(s, "l"), "pc2", "/x", "y"),
        lambda s: s.fs_upload_bytes(lab_id(s, "l"), "pc2", "/x", b"y"),
        lambda s: s.fs_mkdir(lab_id(s, "l"), "pc2", "/d"),
        lambda s: s.fs_move(lab_id(s, "l"), "pc2", "/a", "/b"),
        lambda s: s.fs_copy(lab_id(s, "l"), "pc2", "/a", "/b"),
        lambda s: s.fs_delete(lab_id(s, "l"), "pc2", "/a"),
        lambda s: s.get_machine_api_object(lab_id(s, "l"), "pc2"),
    ],
)
...
def test_mkdir_creates_parents_at_the_normalized_path(service, facade):
    service.fs_mkdir(lab_id(service, "l"), "pc1", "tmp/new//dir")

    assert facade.execs == [("pc1", ["mkdir", "-p", "/tmp/new/dir"], lab_id(service, "l"), False)]


def test_move_and_copy_separate_options_from_paths(service, facade):
    service.fs_move(lab_id(service, "l"), "pc1", "/tmp/-a", "tmp/b")
    service.fs_copy(lab_id(service, "l"), "pc1", "/tmp/-a", "tmp/c")

    assert _commands(facade) == [["mv", "--", "/tmp/-a", "/tmp/b"], ["cp", "-a", "--", "/tmp/-a", "/tmp/c"]]


def test_recursive_delete_uses_rm_rf(service, facade):
    service.fs_delete(lab_id(service, "l"), "pc1", "/tmp/dir", recursive=True)
...
def test_non_recursive_delete_removes_a_file_or_an_empty_directory_with_quoting(service, facade):
    service.fs_delete(lab_id(service, "l"), "pc1", "/tmp/it's here")
...
@pytest.mark.parametrize(
    "call, message",
    [
        (lambda s: s.fs_mkdir(lab_id(s, "l"), "pc1", "/d"), "Create directory `/d` failed on `pc1`: mkdir: denied"),
        (lambda s: s.fs_move(lab_id(s, "l"), "pc1", "/a", "/b"), "Move `/a` failed on `pc1`: mkdir: denied"),
        (lambda s: s.fs_copy(lab_id(s, "l"), "pc1", "/a", "/b"), "Copy `/a` failed on `pc1`: mkdir: denied"),
        (lambda s: s.fs_delete(lab_id(s, "l"), "pc1", "/a"), "Delete `/a` failed on `pc1`: mkdir: denied"),
        (lambda s: s.fs_list_directory(lab_id(s, "l"), "pc1", "/a"), "List directory `/a` failed on `pc1`: mkdir: denied"),
    ],
)
...
def test_a_failing_command_without_stderr_reports_its_exit_code(service, facade):
    ...
    with pytest.raises(ApiError, match=r"Create directory `/d` failed on `pc1`: exit code 2"):
        service.fs_mkdir(lab_id(service, "l"), "pc1", "/d")
...
def test_listing_skips_malformed_records_and_tolerates_unparseable_numbers(service, facade):
    ...
    entries = service.fs_list_directory(lab_id(service, "l"), "pc1", "/srv")
...
def test_listing_sorts_directories_first_then_by_name_case_insensitively(service, facade):
    ...
    entries = service.fs_list_directory(lab_id(service, "l"), "pc1", "/")
...
def test_listing_keeps_names_with_tabs_newlines_and_spaces_whole(service, facade):
    ...
    entries = service.fs_list_directory(lab_id(service, "l"), "pc1", "/root")
...
def test_read_bytes_of_an_empty_file_returns_empty_bytes(service, facade):
    ...
    assert service.fs_read_bytes(lab_id(service, "l"), "pc1", "/empty") == b""
...
def test_machine_api_object_comes_from_the_facade(service):
    assert service.get_machine_api_object(lab_id(service, "l"), "pc1") == ("api-object", "pc1", lab_id(service, "l"))
...
def test_available_shells_falls_back_to_every_shell_when_the_probe_fails(service, monkeypatch):
    ...
    assert service.available_shells(lab_id(service, "l"), "pc1") == ["bash", "sh", "ash", "zsh"]
...
def test_add_link_deploys_it_with_its_external_interfaces(service, facade):
    link = service.add_link(lab_id(service, "l"), "B", external=["eth0", "eth1.20"])

    assert facade.deployed_links == [link]
    assert link.name == "B"
    assert [ext.get_full_name() for ext in link.external] == ["eth0", "eth1.20"]
    assert service.registry.get(lab_id(service, "l")).links["B"] is link


def test_add_link_without_external_interfaces(service, facade):
    link = service.add_link(lab_id(service, "l"), "B")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda s: s.fs_delete(lab_id(s, "l"), "pc1", "/a", recursive=True), ["rm", "-rf", "--", "/a"]),
        (lambda s: s.fs_delete(lab_id(s, "l"), "pc1", "/a"), ["sh", "-lc", "rm -f -- /a || rmdir -- /a"]),
        (lambda s: s.get_startup_log(lab_id(s, "l"), "pc1"), ["cat", "/var/log/startup.log"]),
        (lambda s: s.is_startup_finished(lab_id(s, "l"), "pc1"), ["test", "-f", "/tmp/EOS"]),
        (lambda s: s.available_shells(lab_id(s, "l"), "pc1"), ["sh", "-lc", _SHELL_PROBE]),
    ],
...
def test_each_runtime_operation_runs_one_non_blocking_exec_on_the_right_device(service, facade, call, command):
    ...
    assert facade.execs == [("pc1", command, lab_id(service, "l"), False)]
...
def test_listing_runs_its_script_on_the_right_device_without_blocking(service, facade):
    ...
    assert (machine, lab, wait, command[:2]) == ("pc1", lab_id(service, "l"), False, ["sh", "-lc"])
...
def test_a_startup_log_with_invalid_utf8_is_still_returned(service, facade):
    ...
    assert service.get_startup_log(lab_id(service, "l"), "pc1") == "ip a \ufffd done\n"
...
def test_a_failing_command_with_invalid_utf8_stderr_still_reports_it(service, facade):
    ...
    with pytest.raises(ApiError, match="denied \ufffd"):
        service.fs_mkdir(lab_id(service, "l"), "pc1", "/d")
    with pytest.raises(ApiError, match="denied \ufffd"):
        service.fs_read_bytes(lab_id(service, "l"), "pc1", "/d")
...
def test_a_listing_with_an_invalid_utf8_name_is_still_returned(service, facade):
    ...
    assert [e.name for e in service.fs_list_directory(lab_id(service, "l"), "pc1", "/")] == ["bad\ufffdname"]


def test_a_shell_probe_with_invalid_utf8_still_finds_the_shells(service, facade):
    facade.result = (b"bash\n\xff\nzsh\n", b"", 0)

    assert service.available_shells(lab_id(service, "l"), "pc1") == ["bash", "zsh"]
...
def test_connecting_a_running_device_hands_its_mac_address_to_kathara(service, facade):
    service.connect_machine(lab_id(service, "l"), "pc1", "B", mac_address="02:42:ac:11:00:02")
...
def test_disconnecting_a_running_device_hands_keep_link_to_kathara(service, facade, keep_link):
    if keep_link:
        service.disconnect_machine(lab_id(service, "l"), "pc1", "A", keep_link=True)
    else:
        service.disconnect_machine(lab_id(service, "l"), "pc1", "A")
...
def test_removing_a_running_device_hands_keep_links_to_kathara(service, facade, keep_links):
    if keep_links:
        service.remove_machine(lab_id(service, "l"), "pc1", keep_links=True)
    else:
        service.remove_machine(lab_id(service, "l"), "pc1")
...
def test_connecting_a_stopped_device_appends_the_next_interface_with_its_mac_to_lab_conf(tmp_path, facade):
    ...
    service.connect_machine(lab_id(service, "disk"), "pc1", "B", mac_address="02:42:ac:11:00:02")

    assert service.store.read_lab_conf_text(service.store.lab_dir("disk")) == "pc1[0]=A\npc1[1]=B/02:42:ac:11:00:02\n"
    assert [(n, i.link.name, i.mac_address) for n, i in service.registry.get(lab_id(service, "disk")).machines["pc1"].interfaces.items()] == [
        (0, "A", None),
...
def test_a_failing_recursive_delete_names_the_path(service, facade):
    ...
    with pytest.raises(ApiError) as exc_info:
        service.fs_delete(lab_id(service, "l"), "pc1", "/a", recursive=True)
...
def test_exec_command_by_default_neither_waits_nor_streams(service, facade):
    service.exec_command(lab_id(service, "l"), "pc1", ["ip", "a"])

    assert facade.execs == [("pc1", ["ip", "a"], lab_id(service, "l"), False)]
    assert facade.streams == [False]

...
def test_write_read_layout_round_trip(tmp_path):
    ...
    path = store.write_layout(store.lab_dir("mylab"), LAYOUT)
...
def test_write_layout_overwrites_previous(tmp_path):
    store = LabStore(tmp_path / "labs")
    store.ensure_lab_dir("mylab")
    store.write_layout(store.lab_dir("mylab"), LAYOUT)
    store.write_layout(store.lab_dir("mylab"), {"version": 1, "nodes": {"dev:pc1": {"x": 1, "y": 2}}})
    assert store.read_layout(store.lab_dir("mylab")) == {"version": 1, "nodes": {"dev:pc1": {"x": 1, "y": 2}}}


def test_read_layout_absent_returns_none(tmp_path):
    store = LabStore(tmp_path / "labs")
    store.ensure_lab_dir("mylab")
    assert store.read_layout(store.lab_dir("mylab")) is None
...
def test_write_layout_unknown_lab_raises(tmp_path):
    ...
    with pytest.raises(LabNotFoundError):
        store.write_layout(store.lab_dir("nope"), LAYOUT)
...
def test_read_layout_unknown_lab_raises(tmp_path):
    ...
    store = LabStore(tmp_path / "labs")
    with pytest.raises(LabNotFoundError):
        store.read_layout(store.lab_dir("nope"))


def test_delete_layout_unknown_lab_raises(tmp_path):
    store = LabStore(tmp_path / "labs")
    with pytest.raises(LabNotFoundError):
        store.delete_layout(store.lab_dir("nope"))


def test_delete_layout_reports_whether_it_existed(tmp_path):
    store = LabStore(tmp_path / "labs")
    store.ensure_lab_dir("mylab")
    assert store.delete_layout(store.lab_dir("mylab")) is False
    store.write_layout(store.lab_dir("mylab"), LAYOUT)
    assert store.delete_layout(store.lab_dir("mylab")) is True
    assert not store.layout_path(store.lab_dir("mylab")).exists()
...
def test_service_layout_round_trip(tmp_path):
    ...
    service.save_lab_layout(lab_id(service, "mylab"), LabLayout.model_validate(LAYOUT))
    layout = service.get_lab_layout(lab_id(service, "mylab"))
...
def test_service_ignores_invalid_layout_content(tmp_path):
    ...
    service = make_service(store)
    _lab_with_pc1(service)
    store.write_layout(store.lab_dir("mylab"), {"version": 1, "nodes": {"pc1": {"x": 1, "y": 2}}})
    assert service.get_lab_layout(lab_id(service, "mylab")).nodes == {}
...
def test_clear_lab_layout(tmp_path):
    ...
    _lab_with_pc1(service)
    service.save_lab_layout(lab_id(service, "mylab"), LabLayout.model_validate(LAYOUT))
    assert service.clear_lab_layout(lab_id(service, "mylab")) is True
    assert service.clear_lab_layout(lab_id(service, "mylab")) is False
    assert service.get_lab_layout(lab_id(service, "mylab")).nodes == {}
...
def test_layout_survives_reload_and_zip_round_trip(tmp_path):
    ...
    assert fresh.registry.get(lab_id(fresh, "mylab")) is not None
    assert fresh.get_lab_layout(lab_id(fresh, "mylab")).nodes["dev:pc1"].x == 240

    _name, buf = service.export_lab_zip(lab_id(service, "mylab"))
...
def test_layout_routes(client, tmp_path, monkeypatch):
    ...
    assert client.get(f"/api/labs/{lab_id(service, 'routelab')}/layout").json() == {"version": 1, "nodes": {}}

    saved = client.put(f"/api/labs/{lab_id(service, 'routelab')}/layout", json=LAYOUT)
    assert saved.status_code == 200
    assert client.get(f"/api/labs/{lab_id(service, 'routelab')}/layout").json() == {
        "version": 1,
        "nodes": {"dev:pc1": {"x": 240.0, "y": 130.0}, "cd:A": {"x": 380.5, "y": 210.0}},
    }
    assert store.layout_path(store.lab_dir("routelab")).is_file()

    bad = client.put(f"/api/labs/{lab_id(service, 'routelab')}/layout", json={"nodes": {"pc1": {"x": 0, "y": 0}}})
    assert bad.status_code == 422  # FastAPI body validation (invalid node id)

    assert client.delete(f"/api/labs/{lab_id(service, 'routelab')}/layout").status_code == 200
    assert not store.layout_path(store.lab_dir("routelab")).exists()
    assert client.delete(f"/api/labs/{lab_id(service, 'routelab')}/layout").status_code == 200  # idempotent

    assert client.put(f"/api/labs/{lab_id(service, 'unknown_lab')}/layout", json=LAYOUT).status_code == 404
    ...
    assert client.get(f"/api/labs/{lab_id(service, 'unknown_lab')}/layout").status_code == 404
    assert client.delete(f"/api/labs/{lab_id(service, 'unknown_lab')}/layout").status_code == 404

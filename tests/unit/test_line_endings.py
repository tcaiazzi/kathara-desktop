"""`lab.conf` line endings: the parser and the editor must split identically.

Splitting on `\\r?\\n` in one and `\\r\\n|\\r|\\n` in the other is indistinguishable on LF and CRLF,
so a disagreement shows up only on a bare CR — and there it does not surface as a parse error.
`CONF_LINE_RE`'s `[^"']+` swallows the following line into the value, so a device silently loses
an interface with `errors` and `warnings` both empty, and the mutilated topology is registered as
a successful parse.
...
def test_a_cr_terminated_conf_does_not_swallow_the_next_line():
    """Split differently, `image` comes back as `'kathara/base\\rpc1[0]=A'` with no interfaces,
